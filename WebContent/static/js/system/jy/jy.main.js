	...
	JY={Object:{notNull:function(obj) {//判断某对象不为空..返回true 否则 false
		...
		Dict:{//ids 对应id值(多个逗号分隔).keys 对应key值(多个逗号分隔).type(可选)1.请选择，2.自定义数组。默认不填.dfstr (可选)自定义数组
				setSelect:function(ids,keys,type,dfstr){
					$.ajax({type:'POST',url:jypath+'/backstage/dataDict/getDictSelect',data:{ids:ids,keys:keys},dataType:'json',success:function(data,textStatus){
			        	if(data.res==1){var map=data.obj;var idss= ids.split(",");var opts="",name="";
                            ...
                            else if(type == 3){//dfstr选中的值
                                if(dfstr) {
                                    var values = dfstr.toString().split(",");
                                    if(values)
               ...
               getData:function(keys){
                    var retData = null;
                    $.ajax({type:'POST',url:jypath+'/backstage/dataDict/getDictSelect',data:{ids:keys,keys:keys},dataType:'json', async:false, success:function(data,textStatus) {
                            if (data.res == 1) {
                                retData = data.obj;
                            }
                        }
               ...
               //获取字典翻译
               getName: function(dictDataList, value) {
                    for(var i=0;i<dictDataList.length;i++) {
                        if(dictDataList[i].value == value) {
                            return dictDataList[i].name;
                        }
                    }
                    return "";
               }
		...
		Tags:{//设置按钮用的方法,id 这行的id,pBtn 按钮组
			...
			setFunction:function(id,pBtn){
				...
				if(pBtn!=null&&pBtn.length>0){
					...
					for(var i=0;i<pBtn.length;i++){h+="<a href='#' title='"+JY.Object.notEmpty(pBtn[i].name)+"' onclick='"+JY.Object.notEmpty(pBtn[i].btnFun)+"(&apos;"+id+"&apos;)' class='aBtnNoTD btn btn-white btn-xs' ><i class='"+pBtn[i].icon+" bigger-140'></i></a>";}
					h+="</div>";
		...
		Model:{//loading框
            ...
            edit:function(id,title,savefn,cancelfn){
                $("#"+id).removeClass('hide').dialog({minWidth:350,resizable:false,modal:true,title:"<div class='widget-header'><h4 class='smaller'>"+(JY.Object.notNull(title)?title:"修改")+"</h4></div>",title_html:true,buttons:[{html: "<i class='icon-ok bigger-110'></i>&nbsp;保存","class":"btn btn-primary btn-xs",click:function(){if(typeof(savefn) == 'function'){savefn.call(this);}}},{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(cancelfn) == 'function'){cancelfn.call(this);}}}]});
            },
            //调整
