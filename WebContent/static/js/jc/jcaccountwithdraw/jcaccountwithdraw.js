$(function () {
    ...
    $('#addBtn').on('click', function (e) {
        ...
        //通知浏览器不要执行与事件关联的默认动作
        e.preventDefault();
        var url = jypath + '/jc/jcaccountwithdraw/toAdd';
        art.dialog.open(url, {
    ...
    $('#deleteBatchBtn').on('click', function (e) {
        ...
        if (chks.length == 0) {
            ...
            JY.Model.confirm("确认要删除选中的数据吗?", function () {
                JY.Ajax.doRequest(null, jypath + '/jc/jcaccountwithdraw/deleteBatch', {chks: chks.toString()}, function (data) {
...
function getbaseList(init) {
    ...
    JY.Model.loading();
    JY.Ajax.doRequest("baseForm", jypath + '/jc/jcaccountwithdraw/findWithdrawViewByPage', null, function (data) {
...
function exportExcel() {
    var param = 'withdrawType='+$("#baseForm select[name='withdrawType']").val() + "&keyWord=" + $("#baseForm input[name='keyWord']").val();
    window.location.href = jypath + '/jc/jcaccountwithdraw/exportExcel?'+param;
}