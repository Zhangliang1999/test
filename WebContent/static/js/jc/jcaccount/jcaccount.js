$(function () {
    ...
    $('#addBtn').on('click', function (e) {
        ...
        //通知浏览器不要执行与事件关联的默认动作
        e.preventDefault();
        var url = jypath + '/jc/jcaccount/toAdd';
        art.dialog.open(url, {
    ...
    $('#deleteBatchBtn').on('click', function (e) {
        ...
        if (chks.length == 0) {
            ...
            JY.Model.confirm("确认要删除选中的数据吗?", function () {
                JY.Ajax.doRequest(null, jypath + '/jc/jcaccount/deleteBatch', {chks: chks.toString()}, function (data) {
...
function getbaseList(init) {
    ...
    JY.Model.loading();
    JY.Ajax.doRequest("baseForm", jypath + '/jc/jcaccount/findByPage', null, function (data) {
...
function findBtn(accountId) {
    var url = jypath + '/jc/jcaccount/toAdd?type=find&accountId=' + accountId;
    art.dialog.open(url, {
...
function deleteBtn(id) {
    JY.Model.confirm("确认删除吗？", function () {
        JY.Ajax.doRequest(null, jypath + '/jc/jcaccount/delete', {id: id}, function (data) {
            JY.Model.info(data.resMsg, function () {
                search();
            });
        });
...
function updateBtn(accountId) {
    var url = jypath + '/jc/jcaccount/toAdd?accountId=' + accountId;
    art.dialog.open(url, {
...
function exportExcel() {
    ...
    window.location.href = jypath + '/jc/jcaccount/exportExcel?'+param;
}
//系统充值
function sysRechargeBtn(accountId) {
    var url = jypath + '/jc/jcaccount/toSysRecharge?accountId=' + accountId;
    art.dialog.open(url, {
        title: '系统充值', width: 300, height: 300,
        close: function() {
            search();
        }
    });
